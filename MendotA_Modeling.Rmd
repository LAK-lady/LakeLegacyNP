...
#log-transform data is more normal
hist(water_int_k$extTotP_kgday, breaks=50,
     main="non-transformed",
     xlab="External P loading (kg/day)") #prints hist of ext load term
#data right skewed
hist(log10(water_int_k$extTotP_kgday), breaks=50,
     main="log-transformed",
     xlab="External P loading (kg/day)") #prints hist of ext load term
...
cor<- water_int_k %>% 
  select(CyanoDens_cellsmL, Chla_ugL, #select the columns with data needed
         totextP_kgday_k14, HE_ratio_k7, CyanoDens_cellsmL_k7, Chla_ugL_k7) %>%
  mutate(CD=log10(CyanoDens_cellsmL), Chla=log10(Chla_ugL+0.01), #log-transform
         Pext=log10(totextP_kgday_k14), 
         Pint=log10(HE_ratio_k7), 
         CD_k7=log10(CyanoDens_cellsmL_k7),
         Chla_k7=log10(Chla_ugL_k7+0.01)) %>% #log-transform values
  mutate(CyanoDens_cellsmL=NULL, 
         Chla_ugL=NULL, 
         totextP_kgday_k14=NULL,
         HE_ratio_k7=NULL, 
         CyanoDens_cellsmL_k7=NULL, 
         Chla_ugL_k7=NULL) #remove columns with non-trransformed data
...
plot3d( 
  x=log10(water_int_k$HE_ratio_k7), 
  y=water_int_k$totextP_kgday_k14, 
