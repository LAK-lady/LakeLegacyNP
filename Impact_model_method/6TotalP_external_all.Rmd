...
streams3<- streams2 %>%
  rename(PB_kgday="Pheasant Branch",
         ...
         Pstreams_kgday="All") %>%
  #mutate(Pstreams_kgday=(Pstreams_kgday-(PB_kgday*0.5))) %>% #counterfactual scenario 1
  select(sampledate, Yr, daynum, Pstreams_kgday)
```
...
extP_load <- extP_load %>%
  mutate(Pother_kgday=((Patm_kgday + Pstreams_kgday)*.47), #assign new column for P_other with 47% of total
         Pext_kgday=((Patm_kgday + Pstreams_kgday)*1.47))  #increase P_ext 47% to account for P_other
```
