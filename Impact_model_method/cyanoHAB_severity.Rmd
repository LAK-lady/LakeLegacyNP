...
```{r}
SI_fig3<- ggplot(data=Me_Chl, aes(x=sampledate, y=Chla_ugL)) + #plots Chl-a across date
...
SI_fig4<- a / b #plot a over plot b
SI_fig4 #prints out figure
...
```{r}
Me_Secchi_fig2<- Me_Secchi %>%
  select(sampledate, SD_m) #select fields of interest
Me_cyanoRB_fig2<- Me_cyanoRB %>%
  select(sampledate, Yr, daynum, relbiovol) #selects fields of interest
Figure2_data<- merge(Me_Secchi_fig2, Me_cyanoRB_fig2, by=c("sampledate"),
...
b <- ggplot(data=Figure2_data, aes(x=daynum, y=relbiovol)) + #plots cyano RB across day
  geom_point() + #plots observations as points
  theme_classic(base_size=12) +
  geom_smooth() + #adds regression curve
  labs(x="Day of year", 
...
c) Plot CyanoHAB severity, using Chl-a by day of year...

```{r}
SI_fig6<- ggplot(data=subset(Me_phyto, !Yr==2017), #note all data in 2017 were zeroed due 
...
SI_fig6 #prints out figure
ggsave("Products/SI_figures/Figure6_color.png") #saves plot to Products folder
```

d) Plot CD by day of year...

```{r}
SI_fig7<- ggplot(data=Me_phyto, 
...
SI_fig7 #prints out figure
ggsave("Products/SI_figures/Figure7_color.png") #saves plot to Products folder
```
