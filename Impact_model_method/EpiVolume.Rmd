...
watertemp2<- watertemp %>% #make new data frame
  ...
  filter(daynum>= 189 & daynum <=265) %>% #filter for CyanoHAB season +- 6 days
...
```{r}
ggplot(data=t.d, aes(x=datetime, 
...
Vol_epi<- t.d2 %>%
  ...
  select(sampledate, Yr, daynum, Vol_epi_m3, Thermo_depth_m)
```
...
```{r}
Vol_epi_int <- Vol_epi_int %>% select(date, Vol_epi_m3, Thermo_depth_m)
write.csv(Vol_epi_int, file="Cleaned_data/EpiVolume.csv")
```
