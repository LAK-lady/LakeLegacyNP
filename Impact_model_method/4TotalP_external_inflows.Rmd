...
SIfig8<-ggarrange(A,B,C, ncol=3, nrow=1) #prints plots together
SIfig8 #Prints the figure
ggsave(SIfig8, file="Products/SI_figure8_color.png") #save figure
...
PB_flowP2<- loadReg(formula= TotP_mgL ~ model(7), #total P concentration ~ ln(flow rate) - ln(centroid flow rate)
                    data=PB_P, #data file
...
```{r}
### use rLOADEST package to model P load by flow ###
SMC_flowP2<- loadReg(formula= TotP_mgL ~ model(7), #total P concentration ~ ln(flow rate) - ln(centroid flow rate)
...
```{r}
### use rLOADEST package to model P load by flow ###
YR_flowP2<- loadReg(formula= TotP_mgL ~ model(7), #total P concentration ~ ln(flow rate) - ln(centroid flow rate)
                    data=YR_P, #data file
                    flow="Qout_cfs", #name the field with flow rates
                    dates="sampledate", #name the field with sample date
                    conc.units = "mg/L", #define the units of the response measurements
...
d) calculating average annual stream p load...
...
Inflows_yravg<- Inflows %>%
  filter(Yr>2012 & Yr <= 2018,
         Site=="All") %>%#filter for CyanoHAB season
  group_by(Yr) %>%
  summarize(sum=sum(Flux)) #sum annual P loads 
Inflows_yravg
mean(Inflows_yravg$sum)
sd(Inflows_yravg$sum, na.rm=TRUE)/sqrt(length(Inflows_yravg$sum))
```
