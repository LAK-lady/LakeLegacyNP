...
library(zoo) #needed to convert data files to time series
```
...
```{r}
### loading the data ###
all<- read.csv(file="Cleaned_data/Impact_model_input_data.csv")
...
a) extracting lags and add as new fields...
```{r}
### compute lags and add new fields
inputs<- all %>%
  filter(week_bin %in% daynum_bins) 
  mutate(Chla_k=lag(Chla_ugL, n=1),
         CD_k=lag(CD_cellsmL, n=1),
         Pext_k= lag(Pext_kgday, n=1),
         Pint_k= lag(Int_TotP_kgday, n=1), 
         Pratio_k=lag(avgTotP_HEratio, n=1),
         usedate=as.Date(usedate, format="%Y-%m-%d"))
```

Lags added.

b) converting to time series...

```{r}
### build the time series ###
tseries<- inputs %>%
  select(usedate, Chla_ugL, CD_cellsmL, Pext_kg, Int_TotP_kg, avgTotP_HEratio)
tseries<- read.zoo(tseries)
plot(tseries)
```


# Step 3. Check whether variables are correlated:

```{r}
### select the fields to correlate ###
cor<- inputs %>% #select fields with responses, include columns with predictors
  select(Chla_ugL, Chla_k, #chla and lag
         CD_cellsmL,CD_k, #CD and lag
         Pext_kg, Pext_k,#P_ext,t-k
         Int_TotP_kg, Pint_k,#P_int,t-k
         avgTotP_HEratio,  Pratio_k) %>%
  na.omit() #need to remove any NAs derived from lags

### run the correlation ###
M=cor(cor, method="pearson") #perform correlation using Kendall tau method

### plot the correlation matrix ###
png(height=500, width=500, file="Products/SI_figures/Figure24_color.png", type="cairo")
...
Variable correlations defined.

# Step 3. Perform regressions for Chla:

a) Running first model for Chl-a...
Model: Chla_t ~ Chla_t-k + log10(P_ext,t-k) + error

```{r}
regka_Chla<- glm(formula=Chla_ugL ~ #Chla_t
              #Chla_k + #Chl-a_t-k
                log10(Pext_kg), #P_ext,t-k
              data=inputs) #data frame
summary(regka_Chla)#prints the AIC 
summary(lm(regka_Chla))#prints the R2 
aov(regka_Chla) #prints the sum of squares for the regression
vif(regka_Chla) #prints the VIFs 
png(file="Products/SI_figures/Figure25.png", width=750, height=500) #open png file
layout(matrix(c(1,2,3,4),2,2)) #layout 4 graphs/page
fig25<-plot(regka_Chla) #creates plot
dev.off() #closes the file
par(mfrow=c(1,1))
```

b) Running second model for Chl-a...
Model: Chla_t ~ Chla_t-k + log10(P_ext,t-k) + P_int,t-k + error

```{r}
regkb_Chla<- glm(formula=Chla_ugL ~ #Chla_t
              #lag(Chla_ugL, n=1) + #Chl-a_t-k
                log10(Pext_kg) + 
                Int_TotP_kg, #P_ext,t-k
              data=all) #data frame
summary(regkb_Chla)#prints the glm output
summary(lm(regkb_Chla))#prints the R2 
aov(regkb_Chla)#prints the sum of squares for the regression and residuals
vif(regkb_Chla) #prints the VIFs 
png(file="Products/SI_figures/Figure26.png", width=750, height=500) #open png file
layout(matrix(c(1,2,3,4),2,2)) #layout 4 graphs/page
fig26<-plot(regkb_Chla) #creates plot
dev.off() #closes the file
par(mfrow=c(1,1))
```

c) Running third model for Chl-a...
Model: Chla_t ~ Chla_t-7 + log10(P_ext,t-k) + P_hypo:P_epiP,t-k + error

```{r}
regkc_Chla<- glm(formula=Chla_ugL ~ #Chla_t
            #  lag(Chla_ugL, n=1) + #Chl-a_t-k
                log10(Pext_kg) + 
                lag(avgTotP_HEratio, n=1), #P_ext,t-k
              data=all) #data frame
...
png(file="Products/SI_figures/Figure27.png", width=750, height=500) #open png file
layout(matrix(c(1,2,3,4),2,2)) #layout 4 graphs/page
fig27<-plot(regkc_Chla) #creates plot
dev.off() #closes the file
par(mfrow=c(1,1))
```
...
# Step 4. Run regressions for CD:

a) Running first model for CD...
Model: CD_t ~ CD_t-k + log10(P_ext,t-k) + error

```{r}
regka_CD<- glm(formula=CD_cellsmL ~ #Chla_t
             # lag(CD_cellsmL, n=1) + #Chl-a_t-k
                log10(lag(Pext_kgday, n=1)), #P_ext,t-k
              data=all) #data frame
...
png(file="Products/SI_figures/Figure28.png", width=750, height=500) #open png file
layout(matrix(c(1,2,3,4),2,2)) #layout 4 graphs/page
fig28<-plot(regka_CD) #creates plot
dev.off() #closes the file
par(mfrow=c(1,1))
```

b) Running second model for CD...
Model: CD_t ~ CD_t-k + log10(P_ext,t-k) + P_int,t-k + error

```{r}
regkb_CD<- glm(formula=CD_cellsmL ~ #Chla_t
             # lag(CD_cellsmL, n=1) + #Chl-a_t-k
                log10(lag(Pext_kgday, n=1))+
                lag(Int_TotP_kgday, n=1), #P_ext,t-k
              data=all) #data frame
...
png(file="Products/SI_figures/Figure29.png", width=750, height=500) #open png file
layout(matrix(c(1,2,3,4),2,2)) #layout 4 graphs/page
fig29<-plot(regkb_CD) #creates plot
dev.off() #closes the file
par(mfrow=c(1,1))
```

c) Running third model for CD...
Model: CD_t ~ CD_t-7 + P_ext,t-k + P_hypo:P_epiP,t-k + error

```{r}
regkc_CD<- glm(formula=CD_cellsmL ~ #Chla_t
            #  lag(CD_cellsmL, n=1) + #Chl-a_t-k
                log10(lag(Pext_kg, n=3))+
                lag(avgTotP_HEratio, n=2), #P_ext,t-k
              data=all) #data frame
...
png(file="Products/SI_figures/Figure30.png", width=750, height=500) #open png file
layout(matrix(c(1,2,3,4),2,2)) #layout 4 graphs/page
fig30<-plot(regkc_CD) #creates plot
dev.off() #closes the file
par(mfrow=c(1,1))
```
...
# Step 5. Calculate the partial sum squares for each predictor for each year:

a) calculating source-attributed impact for Chla...
```{r}
YRregk_Chla_2012<- aov(formula=Chla_ugL ~ #Chla_t
              lag(Chla_ugL, n=1) + #Chla_t-k
                log10(lag(Pext_kg, n=3)) + #P_ext,t-k
