...
SIfig16a_data<- Me_Chl %>%
  filter(daynum>= 189 & daynum<= 259)

SIfig16a<- ggplot(data=SIfig16a_data, 
  ...
  annotate('rect', xmin=min(SIfig16a_data$daynum), xmax=max(SIfig16a_data$daynum),
...
SIfig16b_data<- Me_cyano %>%
  filter(daynum>= 189 & daynum<= 259)

SIfig16b<- ggplot(data=SIfig16b_data,
  ...
  theme_classic(base_size=8)+
  theme(axis.text.x=element_text(angle=45))+
  annotate('rect', xmin=min(SIfig16b_data$daynum), xmax=max(SIfig16b_data$daynum),
...
```{r}
SIfig16<- ggarrange(SIfig16a, SIfig16b, ncol=2, nrow=1, widths=c(0.5, 0.5))
SIfig16
ggsave("Products/SI_figures/Figure16_color.png") #saves plot to Products folder
```
...
Chla_CD2<- Chla_CD2 %>%filter(daynum_bin!=0) 

### spot check bin values and correct as needed ###
Chla_CD2<- Chla_CD2[-c(31,38),] #drop extra values
Chla_CD2$daynum_bin[c(28,29,30)]<- c(231, 245, 259)
```
...
(optional) Regress response variables against each other:

```{r}
### regress non-interpolated data ###
rvregress<- cyanoHAB %>% filter_at(vars(Chla_ugL, CD_cellsmL), all_vars(!is.na(.)))

### regressing CD against chl-a ###
SIfig17<- ggplot(data=rvregress, aes(x=Chla_ugL, y=CD_cellsmL/100000)) + 
  geom_point()+
  stat_smooth(method="lm")+
  theme_classic(base_size=12) +
  labs(x="Chlorophyll-a (ug/L)", y="Cyanobacteria density (x10^5 cells/mL)") +
  stat_cor(label.y=10)+
  stat_regline_equation(label.y=9.5)
SIfig17
ggsave(SIfig17, file="Products/SI_figures/Figure17.png")
### 
```
