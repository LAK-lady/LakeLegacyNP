...
```{r}
### packages needed for reformatting data files ###
library(dplyr) #needed for reshaping/reformating data


### packages needed for plotting data ###
library(ggplot2) #needed for plots
library(ggpubr) #needed for graphing multiple plots

### packages needed for statistical analysis ###
...
cyanos<- phyto %>%
  filter(SiteID=="EFL"|SiteID=="BUO"|SiteID=="EMB"|SiteID=="ENN") %>%
...
ft_m<- 0.3048# conversion factor from feet to meters

### reformatting and filtering chl-a data ###
chla<- Chl %>%
  filter(id=="EFL"|id=="BUO"|id=="EMB"|id=="ENN") %>%
...
### identify sampling cutoff depth for epi ###
hist(chla$Depth_m, breaks=20)
...
HL_chla<- chla %>%
  ...
  filter(avgChla_ugL>=0.001) #remove negative and zero values
```
...
fig3a_data<- HL_chla %>%
  filter(daynum>=145 & daynum<=265) #+- 7 days
...
SI_fig3a<- ggplot(data=fig3a_data, aes(x=daynum, y=avgChla_ugL, color=Yr)) + #plots Chl-a across date
  ...
  labs(x="Day of Year", y="Total Chl-a (ug/L)", title="a)", color="Year") +
...
SI_fig3b<- ggplot(data=fig3b_data, aes(x=daynum, y=avgCD_cellsmL, color=Yr))+
  ...
  labs(x="Day of Year", y="log10(Cyanobacteria Density) (cells/mL)", 
       title="b)", color="Year")
...
b) plotting range by site....

```{r}
### plot range of chla ###
Chla_boxplot<- ggplot(Chla_CD, aes(x= SiteID, y=avgChla_ugL)) +
  geom_boxplot() +
  theme_classic() +
  scale_y_log10()+
  labs(x="Site", y="Chl-a (ug/L)")
Chla_boxplot

### plot range of CD ###
CD_boxplot<- ggplot(Chla_CD, aes(x= SiteID, y=avgCD_cellsmL)) +
  geom_boxplot() +
  theme_classic() +
  scale_y_log10()+
  labs(x="Site", y="Cyanobacteria density (cells/mL)")
CD_boxplot
```

Ranges plotted.

c) checking whether response variables correlate...

```{r}
#### plot correlation ###
Chla_CD_plot<- ggplot(Chla_CD, aes(x=log10(avgCD_cellsmL), y=log10(avgChla_ugL), color=Yr)) +
  geom_point() +
  theme_classic() +
  stat_cor(method="pearson") + 
  geom_smooth(method="lm")+
  labs(x="log10(avg CD) (cells/mL)", y="log10(Chl-a) ug/L)", color="Year")
Chla_CD_plot
```
...
Yr_bin<- seq(from=2012, #define year start of study period
             to=2022, #define year end of study period
             by=1) #creates a sequence every 1 year
Yr_bin #prints the expected years of data

### define the observation frequency ###
daynum_bins<- seq(from=145, #define date start of CyanoHAB season +-7 days
                  to=265, #define date end of CyanoHAB season +- 7 days
...
cyanoHAB_bins<- data.frame(Yr=rep(Yr, each=18), #repeat Yr for every daynum 
                           daynum_bin=rep(daynum_bins, times=11)) #repeat daynum for every year
```
