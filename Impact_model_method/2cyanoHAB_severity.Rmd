...
Me_Chl<- Chl %>%
  ...
  filter(lakeid=="ME", #filter for Lake Mendota data
         ...
         daynum >=189 & daynum <=259) %>% #filter for study period +- 7 days
...
Me_cyano<- phyto %>%
  ...
  filter(lakeid=="ME", #filter for Lake Mendota data
         ...
         Yr>=2013 & Yr<=2018, #filter for study period
         daynum >=189 & daynum <=259) %>% #filter for study period +- 7 days) %>% #filter for cyanobacteria data
...
```{r}
SI_fig3<- ggplot(data=Me_Chl, aes(x=daynum, y=Chla_ugL)) + #plots Chl-a across date
...
```{r}
### plot cyanobacteria relative biovolume by sample date ###
a<- ggplot(data=Me_cyano, aes(x=sampledate, y=cyano_relbiovol))+
  ...
  labs(x="Date", y="Cyanobacteria Biovolume 
       (as % total phytoplankton)")
...
SI_fig4<- ggarrange(a,b, nrow=2, ncol=1)
SI_fig4 #print image
...
# Step 4. Estimate missing response values...

a) merging data files...
```{r}
### merge data files ###
Chla_CD<- merge(Me_Chl, Me_cyano, by="sampledate", all=TRUE) 
```

Data files merged.

b) determining transformations...

```{r}
### plot histographs of transformed and non-transformed data ###
par(mfrow=c(2,2)) #change plot parameters
hist(Chla_CD$Chla_ugL, xlab="Chl-a (ug/L)", main="non-transformed")
hist(log10(Chla_CD$Chla_ugL), xlab="Chl-a (ug/L)", main="log10-transformed")
hist(Chla_CD$CD_cellsmL, xlab="Cyano Density (cells/mL)", main="non-transformed")
hist(log10(Chla_CD$CD_cellsmL), xlab="Cyano Density (cells/mL)", main="log10-transformed")
par(mfrow=c(1,1)) #return plot parameters to default
```

Transformations determined.

c) defining and plot the model...

```{r}
### plot the model ###
SI_fig20<- ggplot(data=Chla_CD, aes(x=log10(CD_cellsmL), Chla_ugL)) + 
  geom_point()+
  theme_classic() +
  geom_smooth() +
  labs(x="log10(Cyanobacteria density) (cells/mL)", y="Total Chl-a (ug/L)")
SI_fig20
ggsave(SI_fig20, file="Products/SI_figures/Figure20.png")

### calculate the model outputs ###
reg_Chla<- lm(data=Chla_CD, formula=Chla_ugL~log10(CD_cellsmL))
summary(reg_Chla)
png(filename="Products/SI_figures/Figure21.png")
par(mfrow=c(2,2))
plot(reg_Chla) 
par(mfrow=c(1,1))
dev.off()
...
```{r}
### fill in missing Chla based on CD using model ###
cyanoHAB<- Chla_CD %>%
  mutate(predChla=predict(reg_Chla, .)) %>%
  mutate(Chla_ugL=ifelse(is.na(Chla_ugL), predChla, Chla_ugL),
         Yr.x=NULL, #remove double column
         daynum.x=NULL, #remove double column
         predChla=NULL) %>%#remove predicted column column
  rename(Yr=Yr.y, daynum=daynum.y)
```

Chla plotted against CD.
...
cyanoHAB_bins<- data.frame(Yr=rep(Yr, each=5), daynum_bin=rep(daynum_bins, times=6)) #create data frame with expected year and day number
```
...
df<- data.frame(x=rep(Yr, each=9),y=rep(week_bins, times=6))
df<- df %>%
  rename(Yr=x, week_bin=y)
```
...
cyanoHAB2<- merge(df, cyanoHAB, by.x=c("Yr","week_bin"), 
                      by.y=c("Yr", "daynum_bin"), all.x=TRUE) 
```
