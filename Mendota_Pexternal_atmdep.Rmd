...
# Step 2. Summarize and view the data:

```{R}
precip_sum <- precip %>%
  mutate(daynum=format(sampledate, format = "%j"))
...
was on average `r mean(precip_avg$totP_atm_kgday, na.rm=TRUE)` +- `r (sd(precip_avg$totP_atm_kgday, na.rm=TRUE))/sqrt(length(precip_avg$totP_atm_kgday))` kg/day.
