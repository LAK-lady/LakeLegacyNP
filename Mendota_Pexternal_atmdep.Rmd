...
precip <- precip %>%
  mutate(dryP_atm_mgday = (P_atm_dry*A_lake_m2), #calculate dry P deposited
         wetP_atm_mgday = P_atm_wet*(A_lake_m2*(Precipitation_in*in_m)*m3_L), 
         #calculate wet P deposited
         totP_atm_mgday = dryP_atm_mgday+wetP_atm_mgday) #calculate total P atm
# deposited every day
```
...
ggplot(data=precip, #need to subset data because of big gap between 2000 and 2008
       aes(x=sampledate, y=totP_atm_mgday/1000)) +
  geom_point() +
  geom_line() + 
  theme_classic() +
...
interpolated<- approx( #creates a vector of data that linearly interpolates 
  ...
  precip$totP_atm_mgday, #the predicted values of Hypo P: Epi P ratio
...
totP_atm_interp <- data.frame( #creates a new data frame that has 
  SampleDate = as.Date(interpolated$x), #measurements every 14 days
                             totP_atm_mgday=interpolated$y) #predicted hypo: epi P ratio
plot(totP_atm_interp,
     xlab="Date",
     ylab="Total P deposited from atmosphere (mg/day)") #plots the new time series
