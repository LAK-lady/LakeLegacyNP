...
Total P from atmospheric deposition plotted.
...
precip_dayavg<- precip_interp %>%  
  ...
  summarize(mean=mean(TotP_kgday, #average daily total P loading
                      na.rm=TRUE),
...
```{r}
precip_sum<- precip_interp %>%
  filter(daynum>= 195 & daynum <=255) #filter for CyanoHAB season
write.csv(precip_sum, file="Cleaned_data/Mendota_P_atm.csv")
```