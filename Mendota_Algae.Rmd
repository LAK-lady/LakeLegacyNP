...
```{r}
Chl<- read.csv(file.choose(), header=TRUE) #folder: Original_data, file: ntl38_v5_chl.csv
```

Chlorophyll data loaded. 

c) loading the phytoplankton data...
```{r}
phyto<-read.csv(file.choose()) # choose ntl88_v13_phytoplankton.csv to load
  #the phytoplankton community dataset
```
...
a) filter data for total algal biomass (as Chl-a) by sample date...
...
Me_Chl<- Chl %>%
  ...
  mutate(sampledate=as.Date(sampledate,
         ...
         daynum=as.integer(format(sampledate, "%j")), #adds field with Julian day
  ...
  select(sampledate, daynum, Yr, Mo, MoYr, Chla_ugL) #selects only fields of interest
write.csv(Me_Chl, file = "Cleaned_data/Mendota_Chl.csv") #save new file in folder "Cleaned_data"
```
...
ggplot(data=Me_Chl, 
  ...
  labs(title="Lake Mendota, WI", #plot title
       ...
       y="Total Algal Biomass as Chlorophyll-a (ug/L)", #y axis label
