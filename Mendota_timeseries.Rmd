...
Me_Cyano<- Me_Cyano %>%
  mutate(sampledate=as.Date(sampledate, origin="1899-12-30"), #format as date
         X=NULL) #remove extra column
Me_P_ext<- read.csv(file="Cleaned_data/Mendota_P_allext.csv") #has the external P loading from streams
Me_P_ext<- Me_P_ext %>%
  mutate(sampledate=as.Date(sampledate, origin="1899-12-30"), #format as date
...
```{r}
water<- merge(water, Me_P_ext, by="sampledate")
```
...
date_range<- seq( #creates a vector of sequence data
  min(water$sampledate), #starting at the first sample date
  max(water$sampledate), #ending at the last sample date
...
Chla_interp <- data.frame( #creates a new data frame that has 
  SampleDate = as.Date(interpolated$x), #measurements every 14 days
                             Chla_ugL=interpolated$y) #predicted chla
...
Cyan_interp <- data.frame( #creates a new data frame that has 
  SampleDate = as.Date(interpolated$x), #measurements every 14 days
                             CRB=interpolated$y) #predicted CRB
...
interpolated<- approx( #creates a vector of data that linearly interpolates 
  water$sampledate, #across date
  water$totextP_kgday, #the predicted values of Hypo P: Epi P ratio
  xout=date_range) #for every 14 days
ExtPload_interp <- data.frame( #creates a new data frame that has 
  SampleDate = as.Date(interpolated$x), #measurements every 14 days
                             TotPext_kgday=interpolated$y) #predicted ext load
plot(subset(ExtPload_interp, SampleDate >= "2008/1/1"),
     xlab="Date",
     ylab="Total P external load (kg/day)") #plots the new time series
...
water_int_k<- transform(water_int, HE_P_ratio_k=lag(HE_P_ratio, n=7), #adds column with X_1, t-7 value
                         TotPext_kgday_k=lag(TotPext_kgday, n=14), #adds column with X_2, t-14 value
